...
intros n m p.
rewrite -> plus_assoc.
assert (K: n + m = m + n). {rewrite -> plus_comm. reflexivity. }

assert (H: n + m + p = m + (n + p)). {rewrite -> plus_assoc.  rewrite -> K. reflexivity. }
rewrite -> H. reflexivity. Qed.
...
Theorem mult_rule : forall m n : nat, m + m * n = m * S n.
Proof.
intros n m. induction n as [|n' IHn'].
simpl. reflexivity.
...
Theorem mult_comm : forall m n : nat,
  m * n = n * m.
Proof.
intros n m. induction n as [| n' IHn']. simpl. rewrite -> mult_0_r. reflexivity.
simpl.  rewrite -> IHn'. rewrite -> mult_rule. reflexivity. Qed.
